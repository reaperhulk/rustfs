...
impl Erasure {
    ...
    fn decode_data_with_reconstruction_verification_policy<B: ErasureShard>(
        ...
        if !missing_data_source && shards.len() == self.total_shard_count() {
            // Every data shard was read and bitrot-verified, so there is nothing
            // to reconstruct or verify. Return before the codec borrows the
            // shards mutably: its `ReconstructShard` impl calls `as_mut` on every
            // present shard, and on a `ReadShard::Shared` GET buffer that forces
            // a copy-on-write of the whole shard on every stripe.
            return Ok(());
        }
...
mod tests {
    ...
    #[test]
    fn data_complete_stripe_keeps_shared_shards_untouched() {
        use crate::erasure::codec::workspace::ReadShard;
        for legacy in [false, true] {
            let erasure = Erasure::new_with_options(2, 2, 4096, legacy);
            let encoded = erasure.encode_data(&vec![0x53; 4096]).expect("encode");
            let mut shards: Vec<Option<ReadShard>> = vec![
                Some(ReadShard::Shared(encoded[0].clone())),
                Some(ReadShard::Shared(encoded[1].clone())),
                None,
                None,
            ];
            erasure
                .decode_data_with_reconstruction_verification(&mut shards)
                .expect("data-complete stripe decodes");
            for (index, shard) in shards.iter().take(2).enumerate() {
                let shard = shard.as_ref().expect("data shard present");
                assert!(matches!(shard, ReadShard::Shared(_)), "legacy={legacy}: shard {index} was copied");
                assert_eq!(shard.as_ptr(), encoded[index].as_ptr(), "legacy={legacy}: shard {index} moved");
            }
            assert!(shards[2].is_none() && shards[3].is_none(), "parity slots stay untouched");
        }
    }
}
